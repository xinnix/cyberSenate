  ...
  private async refreshAccessToken(): Promise<boolean> {
    ...
    this.refreshPromise = (async () => {
      try {
        ...
        console.warn('[HTTP] 开始刷新 token, refreshToken:', `${refreshToken?.substring(0, 10)}...`)

        if (!refreshToken) {
          console.warn('[HTTP] 没有 refreshToken，无法刷新')
          return false
        }
        ...
        console.warn('[HTTP] Refresh 响应状态:', response.statusCode)

        if (response.statusCode !== 200) {
          console.warn('[HTTP] Refresh 请求失败')
          return false
        }

        const result = response.data as any
        console.warn('[HTTP] Refresh 响应数据:', result)

        if (!result?.accessToken) {
          console.warn('[HTTP] Refresh 响应中没有 accessToken')
          return false
        }
        ...
        success: (res) => {
          ...
          if (res.statusCode === 401) {
            ...
            this.refreshAccessToken().then((refreshed) => {
              if (refreshed) {
                // Token 刷新成功，重试原始请求
                console.warn('[HTTP] 使用新 token 重试请求')
                ...
                // Token 刷新失败，清除登录状态
                console.warn('[HTTP] Token 刷新失败，清除登录状态')

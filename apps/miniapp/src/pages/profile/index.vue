...
async function onChooseAvatar(event: any) {
  ...
  if (avatarUrl) {
    ...
    try {
      ...
      console.warn('✅ 获取到永久 URL:', permanentUrl)
...
async function getPhoneNumber(event: any) {
  ...
  try {
    ...
    if (res.statusCode === 200 && res.data) {
      ...
      console.warn('📱 手机号授权响应:', response)

      // 后端返回的是 { success: true, data: { phoneNumber: "xxx" } }
      const phoneNumber = response.data?.phoneNumber
      console.warn('📱 phoneNumber 值:', phoneNumber)
      ...
      if (phoneNumber) {
        ...
        // 更新本地存储
        uni.setStorageSync('userInfo', userInfo.value)
        console.warn('✅ 手机号已保存到数据库:', userInfo.value.phone)
